...
class ReceivedFragment : BaseFragment<FragmentApps1Binding>() {
    private val viewModel: AppsViewModel by activityViewModels()
    private val adapter = ReceivedCardAdapter()
    ...
    override fun bindView() {
        ...
        adapter.onSelectToggle = { appInfo ->
        ...
        adapter.onInstallClick = { appInfo ->
            requestInstallApk(appInfo)
        }
        

        adapter.onMoreClick = { appInfo, anchor ->
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.receivedAppsSearch.collect { apkList ->
                ...
                launch {
                    viewModel.selectedApkFilePaths.collect { selectedPaths ->
                        adapter.selectedApks = selectedPaths
                        adapter.notifyDataSetChanged()
                    }
                }

                launch {
                    viewModel.isLoadingReceived.collect { isLoading ->
                        if (isLoading) {
    ...
    override fun onResume() {
        ...
        if (hasManageStoragePermission()) {
            if (adapter.listData.isEmpty()) {
                viewModel.loadReceivedAppsIfNeeded()
            }
        } else {
    ...
    private fun updateEmptyState() {
        // If loading, don't show empty state to avoid flickering
        val isLoading = viewModel.isLoadingReceived.value
        if (isLoading) {
            binding.allEmpty.gone()
            binding.layoutContent.gone()
            return
