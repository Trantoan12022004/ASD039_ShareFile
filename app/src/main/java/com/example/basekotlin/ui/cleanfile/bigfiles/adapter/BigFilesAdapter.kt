package com.example.basekotlin.ui.cleanfile.bigfiles.adapter
...
import com.example.basekotlin.databinding.ItemBigFileBinding
...
class BigFilesAdapter : BaseAdapter<CleanFileItem, ItemBigFileBinding>() {
    ...
    override fun setBinding(
        ...
        return ItemBigFileBinding.inflate(inflater, parent, false)
    }
    ...
    private fun loadThumbnail(binding: ItemBigFileBinding, item: CleanFileItem) {
        when (item.type) {
            ...
            CleanFileType.APK -> {
                binding.imgThumbnail.setImageResource(R.drawable.ic_cat_apps)
            }
            else -> {
                binding.imgThumbnail.setImageResource(R.drawable.ic_clean_big_files)
            }
        }
    }
}
