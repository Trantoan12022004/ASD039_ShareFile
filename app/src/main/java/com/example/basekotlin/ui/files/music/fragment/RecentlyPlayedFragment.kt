...
import com.example.basekotlin.databinding.FragmentRecentlyPlayedBinding
...
class RecentlyPlayedFragment : BaseFragment<FragmentRecentlyPlayedBinding>() {
    ...
    override fun setBinding(
        ...
        return FragmentRecentlyPlayedBinding.inflate(inflater!!, container, false)
    }
    ...
    override fun bindView() {
        ...
        trackAdapter.onTrackClick = { track ->
            if (viewModel.isSelectionMode.value) {
                ...
                // Phát trong phạm vi danh sách recently played
                val currentList = viewModel.recentlyPlayedTracks.value
                var startIndex = 0
        ...
        binding.layoutShuffle.tap {
            val tracks = viewModel.recentlyPlayedTracks.value
            if (tracks.isEmpty()) {
        ...
        binding.layoutSelectAll.tap {
            ...
            val currentTracks = viewModel.recentlyPlayedTracks.value
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.recentlyPlayedTracks.collect { tracks ->
    ...
    // Vuốt trái 1 item để xoá riêng bài đó khỏi lịch sử — KHÔNG xoá file nhạc thật,
    // chỉ xoá bản ghi trong bảng recently_played.
    private fun setupSwipeToRemove() {
        val swipeCallback = object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            override fun getSwipeDirs(
                ...
                // Đang chọn nhiều bài thì khoá vuốt để tránh xung đột với checkbox
                if (viewModel.isSelectionMode.value) {
                    return 0
                } else {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                val removedTrack = trackAdapter.listData[position]
                viewModel.removeFromRecentlyPlayed(removedTrack.id)
            }
        }
        ItemTouchHelper(swipeCallback).attachToRecyclerView(binding.rvMusic)
    }

    private fun showClearHistoryConfirm() {
        ConfirmActionDialog(
            context = requireContext(),
            title = getString(R.string.clear_history_title),
            message = getString(R.string.clear_history_desc),
