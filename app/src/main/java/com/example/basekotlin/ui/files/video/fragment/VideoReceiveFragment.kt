...
class VideoReceiveFragment : BaseFragment<FragmentAllVideoBinding>() {
    ...
    override fun initView() {
        // Cấu hình Grid 3 cột cho tab Received
    ...
    override fun bindView() {
        ...
        adapter.onItemLongClick = { videoInfo ->
            viewModel.enterSelectionMode(videoInfo.filePath)
        }
        adapter.onSelectToggle = { videoInfo ->
            if (viewModel.isSelectionMode.value) {
                viewModel.toggleVideoSelection(videoInfo.filePath)
            } else {
                viewModel.enterSelectionMode(videoInfo.filePath)
            }
        }
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                // 1. Quan sát danh sách video đã nhận
                launch {
                    viewModel.receivedVideosUi.collect { list ->
                ...
                // 3. Quan sát các đường dẫn video đã chọn
                launch {
    ...
    private fun openVideoDetail(video: VideoInfo) {
        val currentList = viewModel.receivedVideosUi.value
        var targetPosition = 0
        for (i in currentList.indices) {
            if (currentList[i].filePath == video.filePath) {
                targetPosition = i
                break
            }
        }
